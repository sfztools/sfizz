...
void Editor::Impl::createFrameContents()
{
...

    {
        ...
        currentTheme_ = theme_->getCurrentName();
        theme_->load(currentTheme_);

        Palette& invertedPalette = theme_->invertedPalette;
        Palette& defaultPalette = theme_->normalPalette;
        ...
        mainView->setBackgroundColor(theme_->frameBackground);
    ...
    if (themeMenu_) {
        auto names = theme_->getNames();
        size_t index = 0;
        for (size_t i = 0; i < names.size(); ++i) {
            UTF8String name = names.at(i);
            themeMenu_->addEntry(name);
            if (name == currentTheme_)
                index = i;
        }
        themeMenu_->setCurrent(index);
    }
}
...
void Editor::Impl::valueChanged(CControl* ctl)
{
    ...
    case kTagThemeMenu:
        {
            if (theme_) {
                int currentIndex = theme_->getCurrentIndex();
                if (currentIndex == -1 || currentIndex == value) {
                    break;
                }
            }
            if (themeMenu_) {
                CMenuItem* item = themeMenu_->getCurrent();
                if (item) {
                    std::string currentTheme = item->getTitle().getString();
                    theme_->storeCurrentName(currentTheme);
